...
func newFreeClientPool(db ethdb.Database, connectedLimit, totalLimit int, quit chan struct{}, wg *sync.WaitGroup, clock mclock.Clock) *freeClientPool {
	pool := &freeClientPool{
		...
		connPool:       prque.New(connPoolCompare, poolSetIndex),
		disconnPool:    prque.New(disconnPoolCompare, poolSetIndex),
...
func (f *freeClientPool) connect(address string, disconnectFn func()) bool {
	...
	if f.connPool.Size() == f.connectedLimit {
		i := f.connPool.Pop().(*freeClientPoolEntry)
...
func (f *freeClientPool) disconnect(address string) {
	...
	e.connected = false
	f.disconnPool.Push(e)
	log.Debug("Client disconnected", "address", address)
}

// logOffset calculates the linear time-dependent offset for the logarithmic
// representation of recent usage
func (f *freeClientPool) logOffset(now mclock.AbsTime) int64 {
	return f.logOffsetAtStartup + (int64(now-f.startupTime))/int64(recentUsageExpTC/fixedPointMultiplier)
}
...
// connPoolCompare is the comparison function of the priority queue of connected nodes.
// It puts the node with the highest recent usage to the front in order to be able to
// compare it to the usage value of incoming connections and kicking it out if necessary.
func connPoolCompare(i, j interface{}) bool {
	return i.(*freeClientPoolEntry).linUsage-j.(*freeClientPoolEntry).linUsage > 0
}

// disconnPoolCompare is the comparison function of the priority queue of known but
// currently disconnected nodes. It puts the node with the lowest recent usage to the front
// in order to drop them if the disconnected pool grows over the size limit.
func disconnPoolCompare(i, j interface{}) bool {
	return i.(*freeClientPoolEntry).logUsage-j.(*freeClientPoolEntry).logUsage < 0
}
